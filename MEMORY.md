# Long-Term Memory

## 🗳️ Political Monitoring — Johor PRN 2026

### N41 Puteri Wangsa (P158 Tebrau) — Tier-1 Urban Battleground

**Signal Date:** 2026-06-27  
**Escalation:** HIGH  
**Contest:** Five-cornered fight (PH, BN, MUDA, Bersama, Independent)

**Key Dynamics:**
- MUDA won in 2022 (Amira Aisya, 7,114 majority) but incumbent NOT defending
- PH deploying Dr Maszlee Malik (former Education Minister) — signals serious intent to reclaim
- MUDA fielding Rashifa Aljunied (26, Chief of Staff to MUDA President) — youth appeal play
- BN/MCA candidate Teow Chia Ling focusing on local-service credibility (congestion, public facilities)
- Bersama entering progressive southern Johor seat via Nicholas Paul Vincent

**Demographics:**
- 128,723 voters (2nd highest in Johor)
- Chinese 51.4%, Malay 35.7%, Indian 10.4%
- Youth 18–29: 35.5% (44,349 voters) — structurally decisive
- 2022 turnout only 47.9% vs 86.9% in 2018 — mobilisation is the battleground

**Critical Issues:** Flooding/drainage, traffic congestion, youth representation, urban services

**Assessment:** Not a straightforward PH–MUDA contest. Fragmented urban-progressive seat where turnout, youth mobilisation, Chinese voter consolidation, and service-delivery credibility will determine margin. MUDA faces retention challenge without incumbent's personal mandate.

---

### N16 Sungai Balang (P146 Muar) — Tier-2 BN-Defensive / Contestable

**Signal Date:** 2026-06-27  
**Escalation:** MEDIUM  
**Contest:** Three-cornered fight (BN, PH, PN)

**Seat Snapshot:**
- Incumbent: Selamat Takim (BN / UMNO), re-fielded
- PH candidate: Ayna Soraya Badaruddin (PKR)
- PN candidate: Muhammad Amin Sailan (PAS)
- 2022 electorate: 30,121 registered voters
- 2022 turnout: 61.8% (vs 84.9% in 2018)
- 2022 majority: 2,293 votes / 12.6% margin

**Historical Pattern:**
- Long-standing BN/UMNO seat
- 2018: BN won by only 174 votes (0.9% margin) with 84.9% turnout
- 2022: BN expanded margin to 12.6% with split opposition and low turnout
- Structurally BN-leaning Malay-majority, but PH came very close in 2018

**Demographics:**
- Malay: ~73%
- Chinese: ~26%
- Indian/Others: <1%
- Gender: Female 50.5%, Male 49.5%
- Youth 18–29: ~26.1%
- Muar coastal–semi-rural belt: kampung, fishing, agriculture, small-town

**Strategic Assessment:**
- **BN**: Defensive position with incumbency advantage. Main risk is Malay vote leakage to PN combined with any PH recovery in mixed localities. Safest route is service-delivery campaign around Selamat Takim's accessibility.
- **PH**: Underdog but not irrelevant. Path requires high-turnout mobilisation, Chinese turnout recovery, women/youth engagement, and targeted Malay persuasion. Must rebuild from weak 2022 base.
- **PN**: Main disruptor to BN's Malay base. Best route is framing itself as sharper Malay-Muslim alternative, especially where voters are dissatisfied with BN's local machinery.

**Critical Issues:**
- Cost of living (high salience among kampung, fishing, small trader, B40 households)
- Local infrastructure (roads, drainage, flood mitigation, village facilities, coastal access)
- Agriculture / fisheries (coastal-rural livelihood base)
- Youth employment
- Service delivery (incumbent performance and local accessibility)
- Malay-Muslim representation (BN vs PN competition axis)
- Chinese voter turnout (influences PH's ceiling and BN's margin protection)

**War Room Assessment:**
- BN Retention Probability: High, but not uncontested
- PH Upset Probability: Low to moderate; dependent on turnout and Malay leakage from BN
- PN Spoiler/Challenger Role: High
- Key Battleground: Malay-majority kampung and mixed polling districts with 2018 BN-PH narrow margins
- Primary Watch Metrics: 
  - Whether PN can exceed its 2022 base
  - Whether PH can recover beyond 2022 collapse
  - Polling district-level turnout modelling
  - Malay vote split tracking
  - Chinese turnout monitoring
  - Incumbent service-sentiment mapping

**Operational Priority:** Local service politics is central. Village-level familiarity, welfare delivery, religious/community networks, and candidate accessibility are more decisive than abstract state-level messaging.

**Bottom Line:** N16 should be treated as a BN-defensive seat with meaningful PN disruption risk. PH's path is difficult but not structurally impossible because the seat was extremely close in 2018. The decisive variable is whether the 2026 contest becomes a BN-versus-PN Malay consolidation fight, or whether PH can reinsert itself through turnout, mixed-area mobilisation, and targeted local issue campaigning.

**War Room Brief:** Full demographic breakdown saved to `memory/n16-sungai-balang-war-room-brief-20260627.md` (19 PD analysis, tier strategy, vote projections)

---

### N17 Semerah (P147 Parit Sulong) — Tier-2 BN-Leaning / Contestable

**Signal Date:** 2026-06-27  
**Escalation:** MEDIUM  
**Contest:** Three-cornered fight (BN, PH, PN)

**Seat Snapshot:**
- Incumbent: Mohd Fared Mohd Khalid (BN / UMNO), re-fielded — Johor EXCO for Islamic Religious Affairs
- PH candidate: Mohd Khuzzan Abu Bakar (PKR) — former Semerah ADUN and former Johor EXCO (won 2018)
- PN candidate: Halim Kepol (PAS)
- 2026 electorate: 47,431 registered voters (youth 18–29: 24.1%)
- 2022 majority: 4,041 votes / 14.5% margin
- 2022 turnout: ~60–61%
- Nomination Centre: MRSM Batu Pahat (June 27, 2026)
- Federal Presence: PH Communications Director Fahmi Fadzil accompanied Khuzzan — signals strategic priority

**Historical Pattern:**
- 2018: PH won by only 98 votes (0.3% margin) with 84% turnout
- 2022: BN recovered with 14.5% majority on lower turnout (~60%)
- Seat flipped PH→BN between 2018–2022; highly turnout-sensitive

**Demographics:**
- Malay: 75.1% (35,633 voters)
- Chinese: 23.2% (11,027 voters)
- Indian: 0.5% (256 voters)
- Youth 18–30: 24.7% (11,722 voters)
- Gender: Female 51.0%, Male 49.0%
- 26 polling districts across Batu Pahat–Parit Sulong rural/semi-rural belt

**Strategic Assessment:**
- **BN**: Incumbency + EXCO portfolio advantage. Strong rural Malay base (Peserai, Parit Maimon, Kampung Bintang = 51% of voters). Campaign: "Maju Johor" stability narrative + direct access via EXCO portfolio. Risk is Malay leakage to PN compressing margin.
- **PH**: Mohd Khuzzan is known local figure with 2018 win, but must reverse 2022 collapse (12,619 → 6,265 votes). Federal backing evident (Fahmi Fadzil at nomination). "Nostalgia play" — leverage 2018–2022 tenure to re-engage non-Malay voters who sat out 2022. Depends on Chinese turnout recovery (23.2% of electorate) and mixed PD performance.
- **PN**: Structural disruptor with 8,501-vote 2022 base. PAS brand resonates in rural Malay settings. Deep-rooted local PAS branches in semi-rural PDs (Lubok, Parit Kuda). If Halim Kepol exceeds 2022 base, BN margin collapses.

**The Split Math:** 2022 combined opposition vote (PN 8,501 + PH 6,265 = 14,766) exceeded BN's 12,542 by 2,224 votes. BN's safety relies entirely on continued opposition split.

**Critical Issues:**
- Malay vote split (BN vs PN) — decisive factor
- Chinese voter turnout — PH ceiling depends on this bloc
- Youth mobilisation (18–30 = 24.7%) — high turnout favours PH, low favours BN machinery
- Incumbent service delivery credibility (Mohd Fared's EXCO portfolio)
- Rural locality networks and religious-community credibility

**War Room Assessment:**
- BN Retention Probability: High (60–65%)
- PH Upset Probability: Low to Moderate (25–30%) — requires high turnout + Chinese consolidation + limited Malay leakage to PN
- PN Spoiler/Challenger Role: High — vote split dynamics can determine winner
- Key Battleground: Malay-majority PDs (Peserai, Parit Maimon) vs Chinese-concentration PDs (Kampung Pantai Timor, Shahbandar, Jalan Jenang, Pasar)
- Primary Watch Metrics:
  - PN/PAS vote share vs 2022 baseline (8,501 votes)
  - Chinese turnout in Kampung Pantai Timor (75% Chinese), Shahbandar (92%), Jalan Jenang (93%), Pasar (94%)
  - Youth turnout (18–30) across all 26 PDs
  - Malay vote split in Peserai, Parit Maimon, Kampung Bintang

**Operational Priority:** Turnout differential is the decisive variable. 2018 high turnout (84%) produced PH win; 2022 low turnout (60%) produced BN recovery. BN's 4,041-vote cushion is vulnerable if PN exceeds 2022 base and PH recovers Chinese/non-Malay turnout.

**Bottom Line:** N17 Semerah is BN-leaning but operationally competitive. BN enters with incumbent, EXCO advantage, and 2022 majority. PH has credible former ADUN but must rebuild lost vote share. PN is main disruptor: if Halim Kepol grows beyond 8,501 votes and converts Malay protest sentiment, Semerah tightens into genuine three-way contest.

**War Room Brief:** Full 26-PD demographic breakdown saved to `memory/n17-semerah-war-room-brief-20260627.md` (turnout sensitivity analysis, PIR alignment, vote projections)

---

### N24 Senggarang (P150 Batu Pahat) — Tier-2 BN-Leaning Underdog / Live Three-Way

**Signal Date:** 2026-06-28 (Corrected)  
**Escalation:** MEDIUM  
**Contest:** Three-cornered fight (BN, PH, PN) — No Bersama entry  
**Comparison:** Roughly level with N17 Semerah — BN-leaning but genuinely in play

**Seat Snapshot:**
- **2026 Electorate:** 38,629 registered voters (confirmed roll)
- **Demographics:** 60% Malay / ~33% Chinese / ~7% Others — Malay-majority with meaningful Chinese minority
- **Youth 18-30:** 28% of electorate (HEADWIND — requires active GOTV drag, not passive expectation)
- **2022 Turnout:** 59% (standalone state election)
- **2022 Majority:** BN +3,912 votes (inflated by Chinese abstention)
- **Polling Districts:** 15

**2026 Candidates (Confirmed):**
- **BN:** Mohd Yusla Ismail (Incumbent ADUN, won 2022)
- **PH:** Onn Abu Bakar (Current Batu Pahat MP)
- **PN:** Datuk Mohd Rashid Hasnon (Former Batu Pahat MP + Former Deputy Speaker)

**Historical Results:**
- **2022 (State, 59% turnout):** BN 9,725 (45%) | PH 5,813 (27%) | PN 5,624 (26%) — BN +3,912
- **2018 (GE, 85% turnout):** PH (Amanah) won by 3.5% majority
- **Key Pattern:** Every GE year broke 70% turnout; standalone 2022 state = 59%

**Critical Corrections (28 June 2026):**
1. **Chinese share overstated** — Only 17% of seat is genuinely Chinese-tilted (Minyak Beku 52%, BSE Barat 61%, Petani Kechik). Banang (largest PD) is dead-even 47% Malay / 45% Chinese, NOT Chinese area.
2. **Turnout assumption wrong** — 75-78% unrealistic for standalone state; corrected to 64-69%
3. **Youth factor** — 28% youth is HEADWIND (under-turnout in state polls), not tailwind
4. **Rashid profile** — Known local figure (former MP for this area), not outsider. Vulnerability is defector narrative (PKR→Bersatu 2018), not unfamiliarity. S5 (PN upset) upgraded to Moderate probability.

**Revised Turnout Scenarios:**
| Scenario | Turnout | BN % | PH % | PN % | Winner | Margin |
|----------|---------|------|------|------|--------|--------|
| S1: 2022 Repeat | 59% | 45.0% | 25.5% | 24.7% | BN | +4,437 |
| S2: Realistic Baseline | 66% | 44.3% | 29.4% | 26.3% | BN | +3,800 |
| S3: Optimistic State | 70% | 43.5% | 29.6% | 25.9% | BN | +3,750 |
| S4: PH Surge | 70% | 39.9% | 33.3% | 25.1% | BN | +1,800 |
| S5: PN Breakthrough | 70% | 35.5% | 30.3% | 34.2% | **PN** | +40 |
| S6: PH Victory | 75% | 36.2% | 40.7% | 22.4% | **PH** | +1,300 |

**Most Likely Outcome (66% turnout):**
- BN: 11,300 (44.3%) | PH: 7,500 (29.4%) | PN: 6,695 (26.3%)
- **BN wins by +3,800** (reduced from +3,912 in 2022, but seat is LIVE)

**War Room Assessment:**
- **BN Retention Probability:** Moderate-High (60-65%) — Incumbent + rural base, but margin compresses
- **PH Upset Probability:** Low (20-25%) — Ceiling limited by only 17% Chinese-tilted PDs
- **PN Upset Probability:** Moderate (25-30%) — Rashid's local network + defector narrative containment = real path
- **Key Insight:** "In play" means underdog in volatile three-way. **Surprise could be PN, not PH.**

**Critical Thresholds:**
- **BN Win:** Turnout <70% + Malay consolidation >60% + hold Banang PD
- **PH Win:** Turnout >68% + Chinese turnout >75% (in 17% Chinese-tilted PDs) + PN >28%
- **PN Win:** Turnout >68% + PN Malay vote >30% (≈8,500+) + BN <40%

**Key Battleground:** **Banang PD** (largest, 47% Malay / 45% Chinese) — dead-even three-way. Whoever wins Malay moderates here likely wins seat.

**Most Dangerous Variables:**
- BN: Malay split (if PN pulls 30%+ of Malay vote) — Rashid is known local, this is LIVE risk
- PH: Chinese apathy (if turnout stays at 2022 levels) — only 17% Chinese-tilted limits ceiling
- PN: Defector narrative (Rashid's PKR→Bersatu crossover) — attack line is betrayal, not unfamiliarity

**Operational Priority:** Turnout calibration is everything. 2022's +3,912 margin is inflated by Chinese abstention. Realistic 66% turnout = BN wins by ~+3,800, but seat is genuinely competitive because Rashid makes PN a real spoiler threat.

**War Room Brief:** Full turnout scenarios + PD-level targets saved to `memory/n24-senggarang-turnout-scenarios-corrected-20260628.md` (corrected demographics, calibrated hourly tracker, candidate profiles)

---

### N33 Tenggaroh (P158 Mersing) — Tier-2 BN-Leaning / PN Threat

**Signal Date:** 2026-06-27  
**Escalation:** MEDIUM  
**Contest:** Three-cornered fight (BN, PH, PN)

**Seat Snapshot:**
- Registered Voters (2026): 39,001
- Polling Districts: 24
- 2022 Turnout: 57.2%
- 2022 Majority: 1,356 votes (6.3%) — BN retained but margin collapsed
- Incumbent Coalition: BN (switching from MIC to UMNO candidate in 2026)

**2026 Candidates:**
- BN: Mohd Youzaimi Yusof (UMNO)
- PH: Md Yusof Dawam (PKR)
- PN: Muhamad Amerul Muhamad (Bersatu)

**Historical Trend — Margin Collapse:**
- 2013: BN-MIC 61.7% (safe, 85.5% turnout)
- 2018: BN-MIC 25.7% (compressed, 80.4% turnout)
- 2022: BN-MIC 6.3% / 1,356 votes (57.2% turnout)

**2022 Vote Breakdown:**
- BN: 10,528 votes
- PN: 9,172 votes (42.78% of valid vote)
- PKR: 1,529 votes (7.13%)
- Pejuang: 213 votes

**Demographics:**
- Malay: 82.7% (decisive majority)
- Chinese: 13.5%
- Indian: 1.5%
- Bumi Sabah/Sarawak: ~1.6%
- Youth 18–29: 23.7%
- Under 40: 54.4% (structurally decisive)

**Constituency Character:**
- Geography: 1,745 km² (large, dispersed)
- FELDA settlement belt (Tenggaroh 01–07, Tenggaroh Timur 01)
- Jemaluang/Mersing semi-rural communities
- Island polling districts (Pulau Sibu, Pulau Tinggi, Pulau Besar, Pulau Aur, Pulau Pemanggil)
- Economic base: Palm oil/FELDA settler economy, plantation work, smallholder agriculture

**Strategic Assessment:**
- **BN**: Incumbent but vulnerable. UMNO candidate corrects MIC legacy mismatch (82.7% Malay majority). Risks: MIC network demobilisation, Indian/Chinese trust erosion, PN Malay consolidation. Path: hold 2022 base + recover MIC networks + prevent Malay leakage.
- **PN**: Primary challenger with credible path. Already achieved 42.78% in 2022. Simple math: small net swing from BN wins. Path: maintain 2022 base + convert 5–7% BN defection.
- **PH**: Underdog (2022 base only 7.13%). Risk of spoiler effect. Path requires 3x–4x growth + Chinese turnout surge + FELDA/Malay penetration.

**Critical Issues:**
- FELDA second-generation agenda (housing, land, inheritance, youth employment)
- Palm oil income sensitivity (commodity prices → smallholder economics)
- Rural road/service delivery (1,745 km² dispersed constituency)
- Island/coastal logistics (5 island PDs — small numbers matter in tight race)
- Youth retention (54.4% under 40)
- Malay representation sentiment (BN's UMNO switch confirms centrality)

**War Room Assessment:**
- BN Retention Probability: Moderate (55–60%)
- PN Upset Probability: Moderate to High (30–35%)
- PH Breakthrough Probability: Low (10–15%)
- PH Spoiler Effect: High — PH vote share could determine BN vs PN winner

**Key Watchpoints:**
- Malay vote split (BN vs PN) — decisive factor
- FELDA turnout differential (low turnout created 2022 volatility)
- MIC legacy network retention post-candidate switch
- Island PD logistics (Pulau Sibu, Tinggi, Besar, Aur, Pemanggil)
- Youth mobilisation (18–29 = 23.7%)

**Bottom Line:** N33 Tenggaroh is no longer safe BN/MIC fortress. It is a Malay-majority, FELDA-heavy, three-way battleground. BN's UMNO candidate is defensive correction to 2022 near-miss. PN is primary challenger. PH must expand beyond 2022 base or risk being vote-splitter.

**War Room Brief:** Full 24-PD breakdown saved to `memory/n33-tenggaroh-war-room-brief-20260627.md` (FELDA analysis, island PD logistics, vote projections)

---

## 🗳️ N.32 ENDAU (P.154 MERSING) — Full Intelligence Brief (CORRECTED)

**Data As Of:** 2026-06-29 (Candidate Announcement Day) | **Analysis Date:** 2026-06-29  
**Corrections Applied:** 2026-06-29 (DAF Feedback — Historical Results + Strategic Reality Check)

**Seat Snapshot:**
- Total Registered Voters: 28,767
- Polling Districts: 20
- Ethnic Breakdown: Malay 79.6% (22,911), Chinese 13.4% (3,858), Orang Asli 4.9% (1,411), Indian 0.7% (204)
- Age Breakdown: Youth 18–30 = 28.1% (8,084 voters), Under-45 = 61.3% (17,638 voters)
- **Contest:** Four-cornered fight (BN, PH, PN, ASLI)

**Candidates:**
- **BN-UMNO:** Alwiyah Talib (INCUMBENT) — Won 2018 (BN), 2022 (PN), returned to UMNO June 2026
- **PH-PKR:** Saiful Nizam Samat — Unity Government candidate, reform narrative
- **PN:** Hasnul Hakimi Hussien — New candidate, defending without incumbent advantage
- **ASLI:** Jati Awang — Parti Orang Asli Malaysia electoral debut, OA representation play

**Key Dynamics:**
- **Malay super-majority (79.6%)** — Seat hinges on Malay vote consolidation vs split
- **Alwiyah incumbency factor** — High personal recognition, 8-year track record, party-switching risk
- **Youth decisive (28.1%)** — Exceeds typical victory margin; favors parties with strong youth outreach
- **Chinese concentration (13.4%)** — Geographically concentrated in 3 PDs; PH ceiling depends on turnout
- **Orang Asli swing bloc (4.9%)** — ASLI candidacy could consolidate OA vote (spoiler potential)
- **FELDA community** — Rancangan FELDA Endau (1,137 voters, 98.9% Malay) — economically sensitive

---

## 📜 Historical Results (CORRECTED)

### 2022 State Election (Standalone, 55% Turnout)
| Party | Candidate | Votes | Share | Note |
|-------|-----------|-------|-------|------|
| **PN (BERSATU)** | Alwiyah Talib | **8,433** | **50.2%** | **WINNER** |
| **BN (UMNO)** | Mohd Youzaimi Yusof | 5,392 | 32.1% | Runner-up |
| **PH (AMANAH)** | (Candidate) | **1,154** | **6.9%** | Distant third |
| **Others (2 candidates)** | — | ~1,200 | ~7.2% | |
| **Majority** | | **3,041 votes** | | Alwiyah (PN) over BN |

**Corrections:**
- ✅ PH got **6.9% (1,154 votes)**, not 12.4% — AMANAH candidate, not PKR
- ✅ BN candidate was **Mohd Youzaimi Yusof**, not "Mohd Fared Mohd Khalid"
- ✅ Five-cornered fight, not four
- ✅ Turnout was **55%**, majority was **3,041 votes**

### 2018 State Election (GE14, 78% Turnout)
- **BN (UMNO):** Alwiyah Talib — Won (first term)
- **Turnout:** **78%** (not 82%)
- **Majority:** Larger than 2022 (comfortable win)

---

## ⚠️ Strategic Corrections (DAF Feedback 2026-06-29)

**Problem 1: PH's 25% Target Is Structurally Impossible**
- Chinese (13.4%) + OA (4.9%) = **18.3% maximum** even at 100% turnout + 100% support
- PH got **6.9% in 2022** (1,154 votes), targeting 25% requires **3x growth**
- **Reality:** PH cannot exceed 12–15% without 15–20% Malay vote (79.6% of seat)
- **Original plan treated Malay heartland as "barely worth showing up to"** — this is wrong

**Problem 2: Anti-Defection Sentiment Benefits PN, Not PH**
- Alwiyah's party-switching hurts **PN**, not PH
- Malay voters fed up with switching go to **PAS/PN**, not PH candidate with no Malay base
- Original analysis assumed anti-defection votes come to PH — **they won't**

**Problem 3: Plan Is Too Heavy for a Lost Cause**
- Seat "can't be won" under normal conditions (BN 70–75% retention probability)
- Yet original plan was resource-intensive, Chinese-focused, assumed 25% ceiling
- **Corrected strategy:** Lighter, Malay-first presence
  - Lock in Chinese + mixed votes quietly
  - Run decent candidate for name recognition **next time**
  - Use campaign to **learn Malay areas** for future run
  - "Keep a presence," "build for next time"

---

**Strategic Assessment (CORRECTED):**
- **Classification:** BN-favoring (70–75% retention probability)
- **BN Path:** Alwiyah consolidates 60–70% of 2022 personal vote + holds rural Malay base
- **PH Path (REALISTIC):** Maximize Chinese turnout (75%+) + **12–15% ceiling** + build for 2030
- **PN Path:** Retain 30–40% of 2022 Alwiyah-PN voters + anti-defection narrative ("Principles over personalities")
- **ASLI Path:** Consolidate 60–70% OA vote (800–950 votes), symbolic + spoiler role
- **Victory Margin:** BN safe (4,000+ votes) if Alwiyah retains personal vote; contestable if PN resilient + ASLI >5%

**Critical Variable:** How do 2022 Alwiyah-PN voters break? `[MEDIUM]`
- 60–70% follow Alwiyah to BN = BN comfortable win
- 30–40% stay with PN = PN spoiler, BN margin compressed

**Full Briefs:**
- Demographics: `memory/n32-endau-demographic-brief-20260629.md`
- Candidates: `memory/n32-endau-candidate-brief-20260629.md`

---

## 📊 Signal Registry Location
- Path: `memory/signals/YYYY/MM/DD-signals.jsonl`
- Schema: Defined in `memory/2026-06-13-political-signal-registry.md`

## Promoted From Short-Term Memory (2026-06-29)

<!-- openclaw-memory-promotion:memory:memory/2026-06-11.md:41:83 -->
- - PIR-4: BN/UMNO opposition - PIR-5: Youth voter sentiment **Skills Enabled:** - news_collection - social_media_monitor - entity_extraction - narrative_tracking ### Next Actions (Phase 1) 1. **Complete frontend setup** - Admin account creation at /setup 2. **Test news collection** - Verify 7 sources are being monitored 3. **Political Signal Registry schema** - Define memory structure (deadline: 2026-06-20) 4. **PIR keyword refinement** - Expand to all 10 PIRs from HEARTBEAT.md 5. **Integration testing** - DeerFlow → Memory files → Heartbeat cycles ### MiroFish Status **Twitter Bug:** ✅ Fixed (2026-06-11 15:15 UTC) - Patched: `oasis/social_platform/platform.py` line 343-346 - Added empty post_table guard before `post_table[-1]["created_at"]` access - Validated: Batch simulation (10 agents, 4 posts) successful **Simulation History:** - `johor-prn-2026-batch-20260526-074859` - 22 agents, 14 posts, 8/9 PIR complete - Validation run: 10 agents, 4 leadership posts, no crashes ### Timeline | Milestone | Target | Status | |-----------|--------|--------| | Phase 1 Approval | 2026-06-11 | ✅ Complete | | DeerFlow Operational | 2026-06-11 | ✅ Complete | | Political Signal Registry | 2026-06-20 | ⏳ In Progress | | Heartbeat Integration | 2026-06-28 | ⏳ Pending | | First Daily Brief | 2026-07-05 | ⏳ Pending | | Sabah GE16 POC | 30-day window | ⏳ Pending kickoff | ### Operator Notes - Gateway requires authentication (first-boot setup at /setup) - Provisioner restarting (AIO sandbox - not needed for news collection) [score=0.888 recalls=4 avg=1.000 source=memory/2026-06-11.md:41-83]

## Promoted From Short-Term Memory (2026-07-01)

<!-- openclaw-memory-promotion:memory:memory/2026-06-28.md:712:747 -->
- --- [score=0.856 recalls=4 avg=1.000 source=memory/2026-06-28.md:137-138]

## Promoted From Short-Term Memory (2026-07-05)

<!-- openclaw-memory-promotion:memory:memory/2026-06-28.md:3:5 -->
- **Day:** Sunday **Session:** Main **Focus:** CVS system-wide mandate + memory harness build [score=0.877 recalls=0 avg=0.620 source=memory/2026-06-28.md:3-5]
<!-- openclaw-memory-promotion:memory:memory/2026-06-28.md:11:11 -->
- **Decision:** Core Truth Validation System (CVS) is now **MANDATORY** across all sessions, all outputs, all agents. [score=0.877 recalls=0 avg=0.620 source=memory/2026-06-28.md:11-11]
<!-- openclaw-memory-promotion:memory:memory/2026-06-28.md:13:15 -->
- **Authority:** DAF **Effective:** 2026-06-28 **Scope:** No exceptions [score=0.877 recalls=0 avg=0.620 source=memory/2026-06-28.md:13-15]
<!-- openclaw-memory-promotion:memory:memory/2026-06-28.md:17:17 -->
- **Rationale:** [score=0.877 recalls=0 avg=0.620 source=memory/2026-06-28.md:17-17]
<!-- openclaw-memory-promotion:memory:memory/2026-06-28.md:23:23 -->
- **Documentation Created:** [score=0.877 recalls=0 avg=0.620 source=memory/2026-06-28.md:23-23]

## Promoted From Short-Term Memory (2026-07-06)

<!-- openclaw-memory-promotion:memory:memory/2026-06-28.md:63:66 -->
- | Component | Status | Notes | |-----------|--------|-------| | Gateway | ✅ Running | PID 673484, QMD backend active | | QMD | ✅ Installed | v2.5.3, configured as memory backend | [score=0.868 recalls=0 avg=0.620 source=memory/2026-06-28.md:63-66]
<!-- openclaw-memory-promotion:memory:memory/2026-06-28.md:67:70 -->
- | Plugins | ✅ Enabled | memory-core, memory-wiki, active-memory | | Truth Validator | ✅ Working | Tested on N17 brief — correctly catches missing tags | | Memory Harness | ✅ Built | 5 scripts + 3 tracking files | [score=0.868 recalls=0 avg=0.620 source=memory/2026-06-28.md:67-69]

## Promoted From Short-Term Memory (2026-07-07)

<!-- openclaw-memory-promotion:memory:memory/2026-06-28.md:71:71 -->
- | Dreaming Cycle | ⏳ Pending | Requires embedding API | [score=0.859 recalls=0 avg=0.620 source=memory/2026-06-28.md:71-71]

---

## 🔀 Repository Separation (2026-07-10)

**Decision:** Split merged Voron-Campaign repository into two focused repositories.

### Repository 1: Voron-Campaign (RMiT Compliance)
- **URL:** https://github.com/ahmadfaurani/Voron-Campaign
- **Focus:** 250 BNM-Regulated Financial Institutions
- **Content:** RMiT compliance campaign, stakeholder collection, battle cards
- **Target:** 143 FIs, 1,001 stakeholders, RM 28.5M-71.5M revenue
- **Files:** 31 (committed 2026-07-10)
- **Status:** ✅ Cleaned and committed

### Repository 2: HOI-Intelligence-Operations
- **URL:** https://github.com/ahmadfaurani/HOI-Intelligence-Operations
- **Focus:** 100 Tier 2 Malaysian Government Agencies
- **Content:** OSINT collection, daily intel briefs (32), agency profiles (144), PRN Johor 2026
- **Target:** 100 agencies, 10 PIRs, daily briefs (23:00 UTC)
- **Files:** 262 (committed 2026-07-10)
- **Status:** ✅ Created and populated

### Backup
- **Location:** `/home/p62operator/.openclaw/workspace-backup-20260710-151547/`
- **Size:** 11MB
- **Contents:** Full pre-separation state (304 files)

### Validation Results
- ✅ Zero data loss (file counts verified)
- ✅ Clean separation (no cross-contamination)
- ✅ Both repos functional on GitHub
- ✅ Backup preserved

### Next Actions
- **RMiT:** Continue Tier 1 bank collection (16 remaining: Citibank, Bank of China, ICBC, Japanese banks, investment banks)
- **HOI:** Resume Tier A agency profiling (15 remaining after 5 completed: MKN, KP, KDN, KKM, LHDN)

## Promoted From Short-Term Memory (2026-07-11)

<!-- openclaw-memory-promotion:memory:memory/2026-07-05.md:46:46 -->
- **Needs Build/Config (9/16):** [score=0.919 recalls=0 avg=0.620 source=memory/2026-07-05.md:46-46]
<!-- openclaw-memory-promotion:memory:memory/2026-07-05.md:4:6 -->
- **Focus:** Expand offensive security tooling from 5 → 16 repos **Operator:** DAF **CVS Status:** Applied (tool names, paths, versions verified) [score=0.862 recalls=0 avg=0.620 source=memory/2026-07-05.md:4-6]
<!-- openclaw-memory-promotion:memory:memory/2026-07-05.md:14:14 -->
- **Tier 1 - Critical AD/Network:** [score=0.862 recalls=0 avg=0.620 source=memory/2026-07-05.md:14-14]
<!-- openclaw-memory-promotion:memory:memory/2026-07-05.md:19:19 -->
- **Tier 2 - Post-Exploitation:** [score=0.862 recalls=0 avg=0.620 source=memory/2026-07-05.md:19-19]

## Promoted From Short-Term Memory (2026-07-13)

<!-- openclaw-memory-promotion:memory:memory/2026-07-05.md:23:23 -->
- **Tier 3 - C2 Expansion:** [score=0.867 recalls=0 avg=0.620 source=memory/2026-07-05.md:23-23]
<!-- openclaw-memory-promotion:memory:memory/2026-07-05.md:27:27 -->
- **Honorable Mentions:** [score=0.867 recalls=0 avg=0.620 source=memory/2026-07-05.md:27-27]

## Promoted From Short-Term Memory (2026-07-14)

<!-- Burn Notice: Dossier content removed 2026-08-02 -->

## Promoted From Short-Term Memory (2026-07-18)

<!-- openclaw-memory-promotion:memory:memory/2026-07-11.md:5:5 -->
- **Topic:** Choosing a name for the assistant — tied to Prometheus/fire identity [score=0.894 recalls=0 avg=0.620 source=memory/2026-07-11.md:5-5]

## Promoted From Short-Term Memory (2026-07-19)

<!-- openclaw-memory-promotion:memory:memory/2026-07-12.md:5:7 -->
- **Date:** 2026-07-12 07:15 UTC **Authority:** DAF approved **Session:** Telegram direct (640442208) [score=0.894 recalls=0 avg=0.620 source=memory/2026-07-12.md:5-7]
<!-- openclaw-memory-promotion:memory:memory/2026-07-12.md:36:36 -->
- ✅ Complete. All files operationally updated. No placeholders. [score=0.894 recalls=0 avg=0.620 source=memory/2026-07-12.md:36-36]

## Promoted From Short-Term Memory (2026-07-25)

<!-- openclaw-memory-promotion:memory:memory/2026-07-19.md:6:6 -->
- Integrated Crawl4AI 0.9.2 into the DeerFlow collection pipeline as the primary scraping engine, with Firecrawl as automatic fallback. All 9 collection scripts patched and tested. Full 25-source run completed: 100% success rate, ~5 min total, 426 headlines, 24 political. [score=0.862 recalls=0 avg=0.620 source=memory/2026-07-19.md:6-6]
<!-- openclaw-memory-promotion:memory:memory/2026-07-19.md:16:19 -->
- | Metric | Firecrawl-only | Crawl4AI+Firecrawl | |--------|---------------|-------------------| | Success Rate | ~80-90% | 100% | | Total Time | ~15-20 min | ~5 min | [score=0.862 recalls=0 avg=0.620 source=memory/2026-07-19.md:16-19]
<!-- openclaw-memory-promotion:memory:memory/2026-07-19.md:20:21 -->
- | Avg per Source | ~40-60s | ~3-8s (Crawl4AI) | | Political Headlines | ~15-20 | 24 | [score=0.862 recalls=0 avg=0.620 source=memory/2026-07-19.md:20-21]

## Promoted From Short-Term Memory (2026-07-26)

<!-- openclaw-memory-promotion:memory:memory/2026-07-20.md:5:7 -->
- **Source:** https://github.com/lopopolo/harness-engineering/blob/trunk/playbooks/repository-review.md **Shared by:** DAF **Context:** Useful framework, no specific application task [score=0.851 recalls=0 avg=0.620 source=memory/2026-07-20.md:5-7]

## Promoted From Short-Term Memory (2026-07-30)

<!-- openclaw-memory-promotion:memory:memory/2026-07-23.md:5:8 -->
- **Date:** 2026-07-23 **Authority:** DAF **Session:** Telegram direct **Focus:** Comprehensive analytical report on the OpenAI/Hugging Face autonomous AI security incident [score=0.901 recalls=0 avg=0.620 source=memory/2026-07-23.md:5-8]

## Promoted From Short-Term Memory (2026-08-01)

<!-- openclaw-memory-promotion:memory:memory/2026-07-25.md:5:7 -->
- **Time:** ~03:20 UTC **Session:** Telegram direct (640442208) **Channel:** Telegram [score=0.894 recalls=0 avg=0.620 source=memory/2026-07-25.md:5-7]
<!-- openclaw-memory-promotion:memory:memory/2026-07-25.md:11:11 -->
- Continued PI-OS discussion with DAF. He asked about integrating PI-OS with ChatGPT. Key points: [score=0.894 recalls=0 avg=0.620 source=memory/2026-07-25.md:11-11]
<!-- openclaw-memory-promotion:memory:memory/2026-07-25.md:27:27 -->
- PI-OS is format, not platform. The ChatGPT+GitHub integration makes the "AI Layer (Processing)" role in the PI-OS architecture achievable without custom scripting — ChatGPT reads the repo, follows the schemas, and writes back. GitHub remains the canonical authority. [score=0.894 recalls=0 avg=0.620 source=memory/2026-07-25.md:27-27]

## Promoted From Short-Term Memory (2026-08-02)

<!-- openclaw-memory-promotion:memory:memory/2026-07-25.md:38:39 -->
- **Time:** ~03:30 UTC **Commit:** d1ce295 — feat: Strategic alignment with DAF professional profile [score=0.887 recalls=0 avg=0.620 source=memory/2026-07-25.md:38-39]
<!-- openclaw-memory-promotion:memory:memory/2026-07-24.md:5:8 -->
- **Time:** 03:56 UTC **Authority:** DAF **Session:** Telegram direct (640442208) **Channel:** Telegram [score=0.880 recalls=0 avg=0.620 source=memory/2026-07-24.md:5-8]
<!-- openclaw-memory-promotion:memory:memory/2026-07-24.md:12:12 -->
- DAF sent the complete Phase 1 specification for a Personal Intelligence Operating System — a structured, tool-independent memory standard for AI-assisted continuity across conversations, decisions, projects, stakeholders, and documents. [score=0.880 recalls=0 avg=0.620 source=memory/2026-07-24.md:12-12]

## Promoted From Short-Term Memory (2026-08-07)

<!-- openclaw-memory-promotion:memory:memory/2026-08-02.md:47:49 -->
- **Authority:** DAF **Target:** Personal background dossier (IC 800220075553, 14-page PDF, received 2026-07-07) **Action:** PERMANENT DELETION — all files and derived content [score=0.857 recalls=0 avg=0.620 source=memory/2026-08-02.md:47-49]

## 🎓 UiTM R.I.S.I.K AI Enablement — Acceptance in Principle (2026-08-07)

**Initiative:** INIT-20260803-002 — UITM-Aras Strategic Collaboration on AI Enablement of the R.I.S.I.K Framework
**Status:** Active / Coordination stage (upgraded from Draft/Framed on 7 Aug 2026)

**Key Development:** Prof. Madya Dr. Suhaimee (Director, UiTM Centre for Media and Information Warfare Studies) responded with agreement in principle to collaborate. Response came 4 days after DAF's 3 Aug proposal — well within the 14-day success metric. UiTM designated 5 team members for coordination:
1. Prof. Madya Dr. Suhaimee (Director) — aimeesavvy@gmail.com / +60 18-668 2659
2. Dr. Mohd Firdauz bin Mohd Fathir — fidfathir.edu@gmail.com / +60 12-366 8027
3. En. Antashah Bin Mohd Nor — decipher.n9@gmail.com / +60 19-275 7753
4. En. Muhd Faiz Bin Fiezal — mhdfaiz781@gmail.com / +60 11-6214 4294
5. En. Al Faliq Bin Ahmad Zahari — faliqzahari182@gmail.com / +60 19-237 8489

**Strategic significance:** This advances R.I.S.I.K from concept (Workstream D) to academically-grounded, AI-enabled national capability. Tri-sector collaboration: UiTM (academic) + Aras (technology) + PMO (government awareness). 5 named staff with contact details signals genuine institutional commitment.

**Next milestones:**
- DAF to initiate contact with UiTM team (ACT-20260807-001, deadline 17 Aug)
- First working session by ~6 September 2026
- Strategic objectives, research scope, operating model aligned within 60 days of working session

**CognitiveOS records:** CONV-20260807-001, DEC-20260807-001, STK-20260807-001–004, ACT-20260807-001–003, COM-20260807-001

## Promoted From Short-Term Memory (2026-08-08)

<!-- openclaw-memory-promotion:memory:memory/2026-08-02.md:5:6 -->
- **Channel:** Telegram direct (640442208) **Authority:** DAF [score=0.862 recalls=0 avg=0.620 source=memory/2026-08-02.md:5-6]
<!-- openclaw-memory-promotion:memory:memory/2026-08-02.md:10:10 -->
## 🛠️ Skunkworks Division — AIRecon Assessment (2026-08-09)

**Status:** APPROVED — Option A (Tracks A–E, full scope)
**File:** `skunkworks/airecon-intern-assessment-20260809.md`

**Updated Hardware Position (2026-08-09 DAF clarifies):**
- 32 units NVIDIA B200
- 12 units NVIDIA A100
- **Assessment:** Massive overprovision. Qwen3.5 122B (48GB requirement) easily handled. Multiple interns can run parallel model instances. Zero hardware bottleneck.

**Verdict:** Conditional YES — approved as research/extension project, NOT as core build-from-scratch.

**Key Constraints Identified:**
- ~~Hardware cliff: 32B+ models (20GB+ VRAM) needed for reliable operation~~ **ELIMINATED — DAF confirms 32x B200 + 12x A100 available**
- Liability risk: Autonomous exploit execution unsuitable for unsupervised intern deployment.
- Maturity gaps: No CI/CD, "learning" is telemetry not ML, no native portfolio integration.

**Recommended Scope (if approved):**
1. Track A: Malaysian context skills pack (15–20 playbooks for gov/CNII)
2. Track B: "Safe Mode" fork — read-only recon + reporting, no autonomous exploitation
3. Track C: GovSec TIP integration bridge (MCP server or API)
4. Track D: Local Malay/Indon security dataset fork
5. Track E: Gov-formatted report generator (JPM/MAMPU standards, bilingual)

**Alternative Options:**
- Option B (narrow): Tracks B+C only — fastest path to demo for CSM/NACSA
- Option C (pivot): Reject AIRecon, build GovSec TIP-native enrichment agent instead
- Option D (learning): Track A only — exposure without production expectation

**Resource Estimate:** ~~RM 8–12K GPU per head OR ~$200/month cloud~~ **ZERO — on-prem A100/B200 cluster handles all models; 3–6 months intern time; 2–4 hrs/week senior supervision.**

---


## Promoted From Short-Term Memory (2026-08-10)

<!-- openclaw-memory-promotion:memory:memory/2026-08-03.md:5:8 -->
- **Time:** ~08:03 UTC **Session:** Telegram direct (640442208) **Channel:** Telegram **Authority:** DAF [score=0.877 recalls=0 avg=0.620 source=memory/2026-08-03.md:5-8]
<!-- openclaw-memory-promotion:memory:memory/2026-08-03.md:12:12 -->
- DAF forwarded the complete email thread for the proposed ELSA–Aras Integrasi collaboration on an AI-assisted revenue recovery and tax compliance initiative for LHDN (Lembaga Hasil Dalam Negeri). DAF asked me to consume the data. [score=0.877 recalls=0 avg=0.620 source=memory/2026-08-03.md:12-12]
