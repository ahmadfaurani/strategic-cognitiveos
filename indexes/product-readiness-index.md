...
|---------|-------------|-------------------|-----------------|-------|------|---------------------|---------------------|
| GovSec TIP | INIT-20260810-003 | Prototype (v3.0 spec, 12 core entities, 147 endpoints, 23 modules) | Demo-ready (CyberDSA Oct 2026) | faurani-jaafar | 11 Phase 2 deferrals (dark web, SIEM, SOAR, SSO, semantic search, AI hunt gen, BYOK, multi-region HA, regulatory monitoring); security audit completed (OWASP Web+LLM Top 10, 54 npm findings); pre-CyberDSA MFA directive in flight (ACT-20260911-004) | CyberDSA Oct 2026 joint launch with CSM | Medium — 11 deferrals are well-scoped but CyberDSA timeline is tight |
| VoronCitadel | INIT-20260811-001 | Production-deployed (v2.0, 45 tables, 5 frameworks, 295 requirements) | Commercial-ready | faurani-jaafar | Single-node deployment (Phase 2: HA); 17 Phase-2 deferrals identified; custom framework builder deferred; pre-CyberDSA MFA directive in flight (ACT-20260911-003) | CSM GTM activation (INIT-20260804-001) | Low — most mature product, production-live |
| chain:SENTRY | INIT-20260811-001 | v4.2 spec delivered Sep 7 (delta vs v4.1 not yet diffed); doc suite now 4/6 categories (Op Plan v1.1 + Cost Centre v1.0 added); NOT deployed (29 commits/40 days behind trunk at v4.1); Implementation 69%, Deployed 47% | Pilot-ready (Phase 0 exit → 77%, Phase 1 exit → 90%) | hadri | 3 Critical Phase 0 blockers (credential rotation, address-security regression, deployment not describable); 1 Should open (investigator outputs); 2 Future (non-EVM tracing, dossier signing); 4 doc categories outstanding (Backlog, Commercial Readiness, Sales/GTM, Governance — ACT-20260908-001); pre-CyberDSA enhancement directive in flight — CIS hardening (web app + VM) + MFA + webhook fixes (ACT-20260911-002, Fuad → Syahir Sep 9) | None identified | High — 3 Critical blockers, credential exposure window open; documentation now ahead of execution; enhancement load added on same capacity |
