...
| chain:SENTRY | INIT-20260811-001 | v4.2 spec delivered Sep 7 (delta vs v4.1 not yet diffed); doc suite now 4/6 categories (Op Plan v1.1 + Cost Centre v1.0 added); NOT deployed (29 commits/40 days behind trunk at v4.1); Implementation 69%, Deployed 47% | Pilot-ready (Phase 0 exit → 77%, Phase 1 exit → 90%) | hadri | 3 Critical Phase 0 blockers (credential rotation, address-security regression, deployment not describable); 1 Should open (investigator outputs); 2 Future (non-EVM tracing, dossier signing); 4 doc categories outstanding (Backlog, Commercial Readiness, Sales/GTM, Governance — ACT-20260908-001) | None identified | High — 3 Critical blockers, credential exposure window open; documentation now ahead of execution |
