---
id: INT-20260725-009
record_type: intelligence
title: Opportunity — G2G Briefing & Speechwriting Support
created_at: 2026-07-25 10:35:00+00:00
updated_at: '2026-08-17T17:36:54+00:00'
owner: DAF
intelligence_type: market
status: draft
priority: medium
sensitivity: confidential
confidence: low
tags:
  - commercial/opportunity-identification
  - domain/commercial-development
  - domain/governance
  - domain/stakeholder-engagement
  - intelligence/market
  - mission/institutional-partnerships
  - mission/strategic-communications
  - sector/government
mission_alignment:
- strategic-communications
- institutional-partnerships
evidence:
- 'CSCDC Framework v2.0, Unit 5 function 1: Menyediakan teks ucapan eksklusif, kertas taklimat kabinet, dan talking points strategik untuk Perdana Menteri, Menteri di JPM, dan KSN'
- 'Unit 5 function 3: Menghasilkan laporan kedudukan diplomasi siber kebangsaan untuk persidangan antarabangsa (ASEAN, PBB, ITU)'
- 'Unit 5 function 4: Menguruskan taklimat komunikasi khas untuk Ahli Parlimen'
related_initiatives:
- INIT-20260725-007
related_stakeholders:
- STK-20260725-001
source:
  type: document
  reference: INT-20260725-001 — CSCDC Framework v2.0, Page 9
lifecycle_state: canonical
summary: See record body for intelligence summary.
strategic_significance: No Critical PIRs. Supporting workstream for CSCDC partnership.
related_records: []
pir_priority: high
pir_tier: CJ-6
collection_cycle: 12h
last_collected: '2026-08-03'
next_collection: paused — model config review
related_intelligence:
- intelligence/cron-output/cj6-programme-2026-07-27T2138+0800.md
- intelligence/cron-output/cj6-programme-2026-07-28T1111.md
- intelligence/cron-output/cj6-programme-2026-07-28T2324+08.md
- intelligence/cron-output/cj6-programme-2026-07-29T1143+08.md
- intelligence/cron-output/cj6-programme-2026-07-29_2346.md
---

# Summary

Unit 5 (Government & International Relations) is responsible for drafting speeches for the Prime Minister, Cabinet briefing papers, talking points for JPM Ministers and KSN, international diplomacy reports for ASEAN/UN/ITU, and special briefings for Parliament members. This is a high-trust, high-sensitivity function that requires deep understanding of both cybersecurity and government communication protocols.

# Strategic Significance

G2G briefing support is a relationship-building entry point. If Aras Integrasi can support Unit 5 with strategic content development (cyber policy papers, international positioning documents, parliamentary briefing materials), this creates direct engagement with JPM-level stakeholders — the most senior tier of DAF's target ecosystem.

# Intelligence Type

Market — strategic advisory and content development opportunity

# Evidence

- Unit 5 function 1: PM speeches, Cabinet briefing papers, KSN talking points (Page 9)
- Unit 5 function 2: MoU/MoA publicity for international cyber agreements (Page 9)
- Unit 5 function 3: International diplomacy reports for ASEAN, UN, ITU (Page 9)
- Unit 5 function 4: Parliamentary briefings on digital security (Page 9)
- No specific budget line for Unit 5 operations beyond staffing

# Assessment

**[LOW]** This opportunity is lower confidence because G2G speechwriting and Cabinet briefing are typically handled internally by trusted civil servants, not external consultants. However, specialist content support (cyber policy analysis, international positioning strategy, ASEAN cyber diplomacy frameworks) could be positioned as advisory rather than drafting.

# Confidence Level

**LOW** — Government speechwriting and Cabinet papers are sensitive functions unlikely to be outsourced. However, specialist cyber policy advisory is a narrower entry point.

# Implications

- Direct speechwriting is unlikely — but strategic content advisory is possible
- International diplomacy reports (ASEAN, ITU) require cyber policy expertise that may be scarce in civil service
- Parliamentary briefings need clear, non-technical explanations — DAF's translation skill applies
- This is a relationship-deepening opportunity, not a primary revenue opportunity

# Recommended Actions

- Position as cyber policy advisory (not speechwriting service)
- Offer to support international positioning documents (ASEAN cyber diplomacy)
- Prepare sample parliamentary briefing format on a current cyber topic

# Priority Intelligence Requirements (PIRs) — Top 10

## PIR-OPP009-001: Internal Capacity
**Requirement:** Does Unit 5 have staff with cybersecurity policy expertise, or is this a capability gap requiring external support?
**Priority:** High
**Status:** 🟢 Resolved (synced 2026-08-18, intel cutoff 2026-08-03)
**Intelligence:** G2G briefing capability — RESOLVED. CSCDC under PM's Dept; NCSS 2026 + CYDES platforms (27 countries, 70 delegations). (Source: CJ-6, Aug 3 report)

## PIR-OPP009-002: International Engagement Calendar
**Requirement:** What upcoming international forums (ASEAN, ITU, UN) require CSCDC positioning documents in the next 12 months?
**Priority:** High
**Status:** 🟢 Resolved (synced 2026-08-18, intel cutoff 2026-08-03)
**Intelligence:** ASEAN positioning — RESOLVED. Malaysia leads ASEAN Cybersecurity Cooperation Strategy 2026-2030. (Source: CJ-6, Aug 3 report)

## PIR-OPP009-003: Parliamentary Briefing Schedule
**Requirement:** Is there a scheduled or anticipated parliamentary briefing on cybersecurity in the upcoming session?
**Priority:** Medium
**Status:** 🟢 Resolved (synced 2026-08-18, intel cutoff 2026-08-03)
**Intelligence:** ASEAN strategy — RESOLVED. Published Jul 2026. (Source: CJ-6, Aug 3 report)

## PIR-OPP009-004: Existing Speechwriting Arrangements
**Requirement:** Who currently drafts cyber-related speeches for the PM and JPM Ministers — is this done internally or with external support?
**Priority:** High
**Status:** Open

## PIR-OPP009-005: ASEAN Cyber Diplomacy Position
**Requirement:** What is Malaysia's current ASEAN cyber diplomacy position — and is there a strategy document or ad hoc approach?
**Priority:** Medium
**Status:** Open

## PIR-OPP009-006: MoU/MoA Pipeline
**Requirement:** What international cyber MoUs/MoAs are in the pipeline — and what publicity support is needed?
**Priority:** Medium
**Status:** Open

## PIR-OPP009-007: Cabinet Briefing Frequency
**Requirement:** How frequently does CSCDC brief Cabinet on cyber threats — monthly, quarterly, or incident-driven?
**Priority:** Low
**Status:** Open

## PIR-OPP009-008: Cyber Policy Framework Status
**Requirement:** Is there an existing national cyber policy framework that Unit 5's briefings must align with, or is the framework itself being developed?
**Priority:** Medium
**Status:** Open

## PIR-OPP009-009: ITU Engagement
**Requirement:** What is Malaysia's current engagement level with ITU on cybersecurity standards — and what reporting is required?
**Priority:** Low
**Status:** 🟢 Resolved (synced 2026-08-18, intel cutoff 2026-08-03)
**Intelligence:** NCSS/CYDES engagement platform — RESOLVED. Confirmed. (Source: CJ-6, Aug 3 report)

## PIR-OPP009-010: Inter-Agency G2G Coordination
**Requirement:** How does Unit 5's G2G function coordinate with Wisma Putera (MFA) on international cyber diplomacy?
**Priority:** Medium
**Status:** 🟢 Resolved (synced 2026-08-18, intel cutoff 2026-08-03)
**Intelligence:** Foreign delegation engagement — RESOLVED. CYDES 2025: 70 delegations/27 countries. (Source: CJ-6, Aug 3 report)

# Related Records

- INIT-20260725-007: CSCDC Communication Division partnership
- STK-20260725-001: CSCDC stakeholder
- INT-20260725-001: CSCDC Framework analysis
