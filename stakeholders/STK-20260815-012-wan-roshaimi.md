---
id: STK-20260815-012
record_type: stakeholder
title: "Wan Roshaimi"
created_at: 2026-08-15T17:42:00+00:00
updated_at: 2026-08-15T17:42:00+00:00
owner: faurani-jaafar
stakeholder_type: government
organisation: CyberSecurity Malaysia (CSM)
role: CSM Chief Technology Officer (CTO)
influence_level: medium
interest_level: high
relationship_status: new
strategic_relevance: "CSM CTO — technology decision-maker. Relevant for technical evaluation of Voron products and CSCDC infrastructure procurement."
engagement_objective: "Technical evaluation pathway for Voron products within CSM/CSCDC infrastructure."
current_position: "Identified as CSM CTO. Newly identified from intelligence cycles."
commitments_by_us: []
commitments_by_stakeholder: []
last_engagement: "None direct — identified via intelligence (CJ-1 leadership watch)"
next_engagement: "TBD — via CSM technical channels"
relationship_owner: faurani-jaafar
related_initiatives:
  - INIT-20260804-001
sensitivity: confidential
tags:
  - domain/government
  - domain/cybersecurity
  - domain/technology
  - priority/high
  - lifecycle/canonical
related_records:
  - ORG-20260815-002
  - ORG-20260815-012
  - INIT-20260804-001
source:
  type: intelligence-verified
  reference: "STK-20260725-001 body text; CJ-1 leadership watch cycles"
lifecycle_state: canonical
---

# Name

Wan Roshaimi

# Role

Chief Technology Officer (CTO) of CyberSecurity Malaysia.

# Organisation

CyberSecurity Malaysia (CSM)

# Strategic Relevance

CSM CTO — technology decision-maker for CSM and the CSM→CSCDC transition. Relevant for:
1. Technical evaluation of Voron products (VoronDRQ, VoronScout, Voron Citadel)
2. CSCDC infrastructure procurement (RM 485K — social listening, content studio, encrypted portal)
3. PQC Sandbox technical architecture

# Influence Level

**Medium** — CTO with technical authority. Procurement and architecture decisions influenced but not solely controlled.

# Interest Level

**High** — Technology infrastructure for CSCDC, PQC Sandbox, cybersecurity product evaluation.

# Engagement Objective

Technical evaluation pathway for Voron products within CSM/CSCDC infrastructure. Engage on CSCDC infrastructure gaps that map to Aras capabilities.

# Current Position

- Identified as CSM CTO
- Newly identified from intelligence cycles
- Likely involved in CSCDC infrastructure procurement planning

# Last Engagement

No direct engagement. Identified through intelligence collection (CJ-1 leadership watch).

# Next Engagement

TBD — via CSM technical channels. Existing MoU provides pathway.

# Relationship Owner

DAF (strategic-level)

# Related Initiatives

- INIT-20260804-001: VoronDRQ / Voron Citadel Productisation (CSM MoU)

# Related Records

- ORG-20260815-002 (CSM)
- ORG-20260815-012 (CSCDC)

# Notes

[VERIFIED] — CJ-1 leadership watch cycles. Technical engagement pathway — CSM MoU provides natural access for product evaluation discussions.
