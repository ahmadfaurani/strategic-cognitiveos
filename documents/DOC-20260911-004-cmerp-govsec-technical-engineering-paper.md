...
- The authorisation fields are **completed form fields (names + dates), NOT a cryptographic digital signature** — the supplied PDF contains no /ByteRange, /SigFlags, or signature SubFilter objects; it is a Quartz print/export of a signed-or-filled form. "Digitally signed" in DAF's brief should be read as *formally acknowledged and dated in the authorisation block*, not PAdES-verified. **RESOLVED by DAF ruling (Telegram, 11 Sep 2026 07:03 UTC): no cryptographically signed variant exists — this form-field acknowledgement IS the signature of record.** External/co-branding claims should therefore say "signed by CSM (Head of MyCERT), 4 Sep 2026 (authorisation block)" — avoid "digitally signed" phrasing that implies cryptographic verification.
