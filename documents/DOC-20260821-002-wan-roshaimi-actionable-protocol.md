...
**TERTIARY: chain:SENTRY** — zero CSM technical integration. Co-branding is a commercial decision.
...
- [ ] VoronCitadel + chain:SENTRY secondary brief prepared
...
**Design principle:** GovSec gets 35 minutes because it has real technical evidence. VoronCitadel + chain:SENTRY get 15 minutes combined because they're secondary. The meeting should feel like "we've been building this with your team" — not "please validate three products."
...
- [ ] VoronCitadel + chain:SENTRY co-branding acknowledged (secondary, partnership-aligned)
...
| **GovSec validated, secondaries questioned** | Proceed with GovSec co-branding. Hold VoronCitadel/chain:SENTRY co-branding pending further technical evidence. DAF assesses commercial positioning. | Hadri + DAF |
...
| 1C.4 | Update Gate 4 status in success metrics dashboard — GovSec / VoronCitadel / chain:SENTRY tracked separately | DAF | Dashboard updated | 48h post-meeting |
...
- VoronCitadel + chain:SENTRY status documented (validated, conditional, or deferred)
...
| Step | Action | Owner | Trigger | Output |
|------|--------|-------|---------|--------|
| 2B.1 | Assess VoronCitadel technical integration opportunity — could VoronCitadel governance/compliance layer sit alongside GovSec in the architecture? | Hadri | Post-2A.3 | Integration opportunity assessment |
| 2B.2 | Assess chain:SENTRY technical integration opportunity — could chain:SENTRY emerging tech layer extend the stack? | Hadri | Post-2A.3 | Integration opportunity assessment |
...
| 3.4 | Identify Aras capabilities that map to CSCDC infrastructure gaps — GovSec as the anchor, VoronCitadel/chain:SENTRY as adjacent capabilities | Hadri | CSCDC gap analysis | Capability mapping document |
...
| Gate 4 (Wan Roshaimi) — VoronCitadel | Validated | Deferred (acceptable) | Declined |
| Gate 4 (Wan Roshaimi) — chain:SENTRY | Validated | Deferred (acceptable) | Declined |
...
**Note:** VoronCitadel and chain:SENTRY Amber = Deferred is acceptable. Gate 4 can close with GovSec Green and secondaries Amber. Gate 4 is Red only if GovSec is Red.
...
| Wan Roshaimi validates GovSec but questions VoronCitadel/chain:SENTRY co-branding | DAF assesses — hold secondaries or proceed commercially | DAF decision | Within 5 days |
...
| Gate 4 — VoronCitadel status documented | Yes/No | Meeting recap |
| Gate 4 — chain:SENTRY status documented | Yes/No | Meeting recap |
...
| VoronCitadel or chain:SENTRY integration path assessed by Dec 31 | Yes/No | Assessment document |
...
1. **GovSec is the primary ask.** Gate 4 leads with GovSec co-branding validation. VoronCitadel and chain:SENTRY are secondary — acknowledged but not the focus.
...
6. **Technical claims must be defensible** — GovSec claims must be backed by the integration evidence pack. VoronCitadel/chain:SENTRY claims must be honestly scoped (GTM-ready vs integration-ready).
